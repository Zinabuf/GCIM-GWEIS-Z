...
pheno <- read.table("trait1_analysis_cov.txt", header = TRUE)
...
table(pheno$trait1)
write.table(pheno, "trait1_analysis_cov_b.txt", col.names = TRUE, row.names = F, quote = F)
#########################################################################
########################################################################
############## recoding the target dataset #############################
# Read continuous data
pheno <- read.table("trait1_PRStrained_cov.txt", header = TRUE)
...
# Check the distribution again to ensure it's approximately 10% 1s and 90% 0s
table(pheno$trait1)
write.table(pheno, "trait1_PRStrained_cov_b.txt", col.names = TRUE, row.names = F, quote = F)