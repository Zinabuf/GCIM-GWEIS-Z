...
#---- Read data ----#
gcim_adj  <- fread("trait2_out_q_gcim_gweis-z.txt")
gcim_raw  <- fread("trait2_out_q_gcim_gweis.txt")
gweis_adj <- fread("trait2_out_q_gweis-z.txt")
gweis_raw <- fread("trait2_out_q_gweis.txt")

#---- Keep only interaction test from raw PLINK outputs ----#
gcim_raw  <- gcim_raw[TEST == "ADDxPRS", ]
gweis_raw <- gweis_raw[TEST == "ADDxtrait1", ]
...
ggsave("Combined_Manhattan_trait2.png", p, width = 10, height = 12, dpi = 600)