...
prs_scores <- function(plink_path,
  ...
  # ---- Handle #FID column name (PLINK2 quirk) ----
  cn <- names(prs_raw)
  if ("#FID" %in% cn && !"FID" %in% cn) {
    names(prs_raw)[names(prs_raw) == "#FID"] <- "FID"
    cn <- names(prs_raw)
  }

  if (!all(c("FID", "IID") %in% cn)) {
    .stop2(
  ...
  # PLINK2 now outputs NAMED_ALLELE_DOSAGE_SUM instead of SCORE1_SUM
  if ("NAMED_ALLELE_DOSAGE_SUM" %in% cn) {
    score_sum_col <- "NAMED_ALLELE_DOSAGE_SUM"
  } else if ("SCORE1_SUM" %in% cn) {
    score_sum_col <- "SCORE1_SUM"
  } else {
    # Fallback: first column that matches SCORE*_SUM or *_DOSAGE_SUM pattern
    cand1 <- grep("^SCORE\\d+_SUM$", cn, value = TRUE)
    cand2 <- grep("_DOSAGE_SUM$", cn, value = TRUE)
    cand <- c(cand2, cand1)  # Prefer dosage columns
    if (length(cand) > 0) score_sum_col <- cand[1]
  }
  
  if (is.null(score_sum_col)) {
    .stop2(
      "Could not find a PRS sum column (e.g., NAMED_ALLELE_DOSAGE_SUM or SCORE1_SUM) in: ", sscore_file,
      "\nAvailable columns: ", paste(cn, collapse = ", ")
    )
  }
