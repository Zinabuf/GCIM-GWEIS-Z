...
prs_scores <- function(plink_path,
  ...
  # PLINK2 now outputs SCORE1_AVG instead of SCORE1_SUM
  if ("SCORE1_AVG" %in% cn) {
    score_sum_col <- "SCORE1_AVG"
  ...
  if (is.null(score_sum_col)) {
    .stop2(
      "Could not find a PRS sum column (e.g., SCORE1_AVG or SCORE1_AVG) in: ", sscore_file,
      "\nAvailable columns: ", paste(cn, collapse = ", ")
    )
  }
