...
#' - Uses --glm interaction hide-covar for logistic model
...
#' @param ensure_12 Logical. If TRUE, recode 0/1 phenotype to 1/2 in a temp file (default TRUE).
...
b_gweis <- function(plink_path,
  ...
  .validate_binary_and_maybe_recode <- function(pheno_df, pheno_col, tmp_dir, ensure_12) {
    ...
    if (length(u) < 2) .stop2("Binary phenotype has <2 unique values. Values: ", paste(u, collapse = ", "))
    if (length(u) > 2) {
  ...
  # ---- Compute explicit covar-col-nums list (avoid ranges) ----
  # (Keep covar_col_range in args/meta for compatibility, but do not rely on it.)
  ...
  if (n_removed_eff > 0) {
    warning("Removing ", n_removed_eff, " SNPs with missing/invalid effect values (", effect_source, ")",
            call. = FALSE)
