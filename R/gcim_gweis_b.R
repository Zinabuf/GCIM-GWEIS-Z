...
b_gweis <- function(plink_path,
  ...
  .validate_binary_and_maybe_recode <- function(pheno_df, pheno_col, tmp_dir, ensure_12) {
    ...
    recoded <- FALSE
    pheno_path_used <- NULL

    if (isTRUE(ensure_12) && identical(u, c(0, 1))) {
      pheno_df[[pheno_col]] <- ifelse(pheno_df[[pheno_col]] == 0, 1,
                                      ifelse(pheno_df[[pheno_col]] == 1, 2, pheno_df[[pheno_col]]))
      pheno_path_used <- file.path(tmp_dir, "pheno_recoded_12.txt")
      utils::write.table(pheno_df, pheno_path_used,
  ...
  if (int_covar_index > n_covars) {
    .stop2("int_covar_index exceeds available covariates (", n_covars, ").")
  }
  ...
  # Predictor order for standard --glm interaction setup:
  # 1 = ADD
  ...
  message("Running PLINK2 binary GWEIS (interaction with all covariates retained)...")
  ...
  if (!"TEST" %in% names(gweis_res)) {
    .stop2("TEST column not found in: ", glm_file)
  }
  ...
  if (is.null(chosen_term)) {
    if (nrow(interaction_rows) == 1) {
      chosen_term <- interaction_rows$TEST[1]
    } else {
  ...
  gweis_int <- interaction_rows[interaction_rows$TEST == chosen_term, , drop = FALSE]
  if (nrow(gweis_int) == 0) {
    .stop2("Chosen interaction term not found after selection: ", chosen_term)
  }

  # ---- Build GCIM output ----
  needed <- c("ID", "A1", "BETA", "SE", "P")
  miss <- setdiff(needed, names(gweis_int))
  if (length(miss) > 0) {
    .stop2("Missing required columns in PLINK interaction output: ",
           paste(miss, collapse = ", "))
  }
  ...
  gcim_df <- data.frame(
    SNP = gweis_int$ID,
    A1  = gweis_int$A1,
    b   = suppressWarnings(as.numeric(gweis_int$BETA)),
    se  = suppressWarnings(as.numeric(gweis_int$SE)),
    p   = suppressWarnings(as.numeric(gweis_int$P)),
    stringsAsFactors = FALSE
  )

  gcim_df <- gcim_df[stats::complete.cases(gcim_df), , drop = FALSE]
  if (nrow(gcim_df) == 0) {
    .stop2("No complete interaction results remained after filtering.")
  }

  out_gcim <- file.path(tmp_dir, out_file)
  sep_used <- if (identical(gcim_sep, "tab")) "\t" else " "

  utils::write.table(gcim_df, out_gcim,
                     row.names = FALSE, col.names = TRUE,
                     quote = FALSE, sep = sep_used)

  message(.bar())
  message("Binary GWEIS completed successfully")
  message("GLM file: ", glm_file)
  message("Interaction term selected: ", chosen_term)
  message("GCIM file: ", out_gcim)
  message(.bar())
  ...
  invisible(list(
    ...
    interaction_covar = covar_name_used,
    parameters_keep = parameters_keep,
    n_total_covariates = n_covars,
    files = list(
      glm = glm_file,
      gcim = out_gcim,
      plink_log = log_file,
