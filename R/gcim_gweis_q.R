...
#' - Prints helpful PLINK stderr/log snippets automatically on error (exit code 8, etc.)
#' - Robust phenotype/covariate reading (accepts headerless files; accepts EID -> IID)
#' - Optional phenotype variance standardization (off by default to avoid PLINK failures)
#' - Optional additional covariate standardization (on by default)
#' - Clearer diagnostics about sample overlap (pheno/covar vs .fam)
#'
...
#' @param pheno_col Integer. Phenotype column number in phenotype file (default 3).
#' @param standardize_covar Logical. Use --covar-variance-standardize (default TRUE).
#' @param standardize_pheno Logical. Use --pheno-variance-standardize (default FALSE; safer).
...
q_gweis <- function(plink_path,
  ...
  # ---- Resolve list input for covar/pheno ----
  pheno_source <- NULL
  covar_source <- NULL
  ...
  # ---- Read covar header (need number of covars) ----
  cov_header <- tryCatch(
  ...
  if (int_covar_index > n_covars) .stop2("int_covar_index exceeds available covariates")

  covar_name_used <- covar_names[int_covar_index]

  # ---- Explicit covar-col-nums list (no ranges) ----
  ...
  # ---- Robust interaction row detection ----
  interaction_rows <- gweis_res[grepl("^ADDx", gweis_res$TEST), , drop = FALSE]

  if (nrow(interaction_rows) == 0) {
    .stop2(
      "No interaction rows found in PLINK output (no TEST starting with 'ADDx').\n",
      "This usually means PLINK did not emit interactions.\n",
      "Available TEST values: ", paste(unique(gweis_res$TEST), collapse = ", "), "\n",
      "GLM file: ", glm_file
    )
  }

  interaction_terms <- sort(unique(interaction_rows$TEST))
  chosen_term <- if (length(interaction_terms) == 1) interaction_terms[1] else {
  ...
  utils::write.table(gcim_data, file = gcim_file, quote = FALSE, col.names = TRUE,
                     row.names = FALSE, sep = " ")
  ...
  res <- list(
    ...
    meta = list(step = "GWEIS", trait_type = "quantitative", model = "linear_interaction",
                int_covar_index = int_covar_index, int_term = chosen_term,
                covar_cols = covar_cols, timestamp = Sys.time())
  )
