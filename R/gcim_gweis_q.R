...
#'   (COVAR1 is column 3, should be PRS). Can also pass output from `replace_covariate_with_prs()`.
...
q_gweis <- function(plink_path,
  ...
  if (length(glm_files) == 0) {
    .stop2(
      "No .glm.linear output found in: ", tmp_dir,
  ...
  if (!all(keep)) {
    warning("Removing ", n_removed, " SNPs with missing values in key columns", call. = FALSE)
  ...
  if (n_removed > 0) {
    message("  SNPs removed (missing data): ", n_removed)
  }
