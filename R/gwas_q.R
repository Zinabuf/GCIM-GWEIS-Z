...
q_gwas <- function(plink_path,
  ...
  message("Running PLINK2 GWAS (linear regression)...")
  ...
  if (length(glm_files) == 0) {
    # Fallback: any glm output if linear suffix differs
  ...
  # Remove SNPs with missing BETA values
  ...
  if (nrow(add_res) == 0) {
    .stop2("No valid SNPs remain after filtering missing BETA values.")
  }
