...
gcim_z_adjust <- function(glm_file,
  ...
  n_sig_original <- sum(a$P < 0.05, na.rm = TRUE)
  n_sig_adjusted <- sum(a$p_value_int_adj < 0.05, na.rm = TRUE)
  n_sig_bonf_orig <- sum(a$P < 0.05 / nrow(a), na.rm = TRUE)
  ...
  message("  Original_genomewide_threshold:  ", n_sig_original_genome_wide_threshold)
  message("  Adjusted_genomewide_threshold:  ", n_sig_adjusted_genome_wide_threshold)
