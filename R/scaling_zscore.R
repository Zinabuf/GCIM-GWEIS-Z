...
gcim_z_adjust <- function(glm_file,
  ...
  # ---- Compute genome-wide significance statistics ----
  n_sig_original_genome_wide_bonf <- sum(a$p_original < 5e-8, na.rm = TRUE)
  n_sig_adjusted_genome_wide_bonf <- sum(a$p_value_int_adj < 5e-8, na.rm = TRUE)
  ...
  message("")
  message("Bonferroni-corrected for number of SNPs(p < ", sprintf("%.2e", 0.05 / nrow(a)), "):")
  message("  Original:  ", n_sig_bonf_orig)
  message("  Adjusted:  ", n_sig_bonf_adj)

  message("")
  message("Genome-wide significance threshold(p < 5e-8):")
  message("  Original_genomewide_bonf:  ", n_sig_original_genome_wide_bonf)
  message("  Adjusted_genomewide_bonf:  ", n_sig_adjusted_genome_wide_bonf)
