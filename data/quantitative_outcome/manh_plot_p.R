...
#---- Read data ----#
gcim_adj  <- fread("tBil_bmi_gcim-gweis-z.txt")
gcim_raw  <- fread("tBil_bmi_gcim-gweis.txt")
gweis_adj <- fread("tBil_bmi_gweis-z.txt")
gweis_raw <- fread("tBil_bmi_gweis.txt")

#---- Keep only interaction test from raw PLINK outputs ----#
gcim_raw  <- gcim_raw[TEST == "ADDxPRS", ]
gweis_raw <- gweis_raw[TEST == "ADDxbilirubin", ]
...
ggsave("Combined_Manhattan_proposed.png", p, width = 10, height = 12, dpi = 600)