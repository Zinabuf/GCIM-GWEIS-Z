...
#---- Read data ----#
gcim_adj  <- fread("bmi_tBil_gcim-gweis-z.txt")
gcim_raw  <- fread("bmi_tBil_gcim-gweis.txt")
gweis_adj <- fread("bmi_tBil_gweis-z.txt")
gweis_raw <- fread("bmi_tBil_gweis.txt")

#---- Keep only interaction test from raw PLINK outputs ----#
gcim_raw  <- gcim_raw[TEST == "ADDxPRS", ]
gweis_raw <- gweis_raw[TEST == "ADDxBMI", ]
...
ggsave("Combined_Manhattan_rev.png", p, width = 10, height = 12, dpi = 600)